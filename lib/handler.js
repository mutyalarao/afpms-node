...
var addVet = function(req,res,callback)
{
	...
	connection.query(sql,function(err,result){
		if(!err){
			...
			{				
			if (req.body.vetDecorData||req.body.vetPhoneData) 
			{
				...
				/*vetAsync.pushF(function(data,callback){
					for(var z=0;z< vetDecorData.length;z++)
					{
						console.log(z+"-inside phone func");
						var vetDecorSql=vetDecorMap.getInsert(vetDecorData[z]);
						connection.query(vetDecorSql,callback(err,data,vetAsync));
					}
				}
				//var vetDecorSql=vetDecorMap.getInsert(req.body.vetDecorData);
					
				);*/
				
								//vetAsync.addCallback(function(){console.log("")});
				vetAsync.exec(function(err,data){callback(err,data);});				
				
				//var vetPhoneSql=vetDecorMap.getInsert(req.body.vetPhoneData);
				//connection.query(vetPhoneSql,function(err,result){console.log('Inserted Phones'+result.affectedRows);});
			}
